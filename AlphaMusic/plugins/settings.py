...
search_buttons = [
    [
        Button.switch_inline("búsqueda de Pista   🎧", same_peer=True),
        Button.switch_inline("Buscar álbum 💽", query=".a: ", same_peer=True),
    ],
    [Button.inline("❌")],
]
...
async def settings(event: Union[NewMessage.Event, CallbackQuery.Event]):
    ...
    await answer(
        "Settings:",
        buttons=[[Button.inline("Calidad 🎧", data="q")], [Button.inline("❌")]],
    )
