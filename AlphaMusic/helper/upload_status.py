...
class UploadStatus:
    ...
    async def start(self) -> None:
        ...
        self._message = await self._event.reply("Subiendo...")
            ...
            while True:
                if self._total:
                    ...
                    if self._track_count:
                        msg += f" 💿 Pista {self._track_count} de {self._total_tracks}\n"
                    msg += (
                        f"🔼 Subiendo... {(self._current / self._total):.1%}\n"
                        f"⚡ Velocidad: {get_readable_file_size(self.speed())}/s"
                    )
                    try:
